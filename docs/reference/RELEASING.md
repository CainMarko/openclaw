...
summary: "Step-by-step release checklist for npm + portable desktop artifacts"
read_when:
...
# Release Checklist (npm + portable desktop artifacts)
...
## Portable artifact matrix

Use these names for release artifacts attached to tag releases:

| Platform | Artifact name                     | Contents                                    |
| -------- | --------------------------------- | ------------------------------------------- |
| macOS    | `OpenClaw-<version>-macos.zip`    | `OpenClaw.app` plus guided launcher file    |
| Windows  | `OpenClaw-<version>-windows.zip`  | Portable launcher plus onboarding bootstrap |
| Linux    | `OpenClaw-<version>-linux.tar.gz` | Portable launcher plus onboarding bootstrap |

Each artifact should also have:
...
- [ ] Build portable artifacts:
  - [ ] `pnpm dist:package:linux`
  - [ ] `pnpm dist:package:windows`
  - [ ] `pnpm dist:package:macos` (runs existing mac packaging flow first)
- [ ] Verify each portable archive has matching `.manifest.json` and `.sha256` output files.
...
- [ ] Attach release artifacts:
  - [ ] `OpenClaw-X.Y.Z.zip` and `OpenClaw-X.Y.Z.dSYM.zip` (if generated by Sparkle flow)
  - [ ] `OpenClaw-X.Y.Z-macos.zip` + checksum + manifest
  - [ ] `OpenClaw-X.Y.Z-windows.zip` + checksum + manifest
  - [ ] `OpenClaw-X.Y.Z-linux.tar.gz` + checksum + manifest
  - [ ] `npm pack` tarball (optional)
- [ ] Commit the updated `appcast.xml` and push it (Sparkle feeds from main).
- [ ] Verify desktop "download + run" install path:
  - [ ] Extract each archive on its target OS
  - [ ] Launch using bundled launcher
  - [ ] Run onboarding bootstrap and confirm it reaches configuration prompts
